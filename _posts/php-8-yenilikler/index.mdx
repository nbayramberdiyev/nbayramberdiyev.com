...
-   [`+`/`-` ve `.` Operatörleri İşlem Önceliği](#--ve--operatörleri-i̇şlem-önceliği)
-   [Dinamik Port Desteği](#dinamik-port-desteği)
...
PHP 8'den itibaren bu ve benzeri durumlarda parametreleri istediğimiz sırada gönderebiliriz:
...
Artık dockblock ekleyerek bunu manuel parse etmek yerine **PHP Attributes** kullanabiliriz:
...
## `+`/`-` ve `.` Operatörleri İşlem Önceliği

Bir ifade içinde parantez olmadan `+`/`-` ve `.` operatörleri beraber kullanıldığında işlem önceliği `+`/`-` operatörünün olacak.

PHP 7.4, bu tarz kullanımlarda "Deprecated" uyarısı veriyordu:
...
// Deprecated: The behavior of unparenthesized expressions containing both '.' and '+'/'-' will change in PHP 8: '+'/'-' will take a higher precedence in... (PHP 7.4)
```

PHP 8'de herhangi bir uyarı olmayacak ve `+`/`-` operatörü işlem önceliğine sahip olacak:
...
## Dinamik Port Desteği

Eskiden PHP built-in web sunucusunu başlatırken port belirtmemiz gerekiyordu:

```bash
$ php -S localhost:8000

[Fri Dec 11 13:46:30 2020] PHP 8.0.0 Development Server (http://localhost:8000) started
```

PHP 8'den itibaren port olarak `0` yazdığımızda sistemde kullanılmayan rastgele bir port üzerinde sunucu başlatılacaktır:

```bash
$ php -S localhost:0

[Fri Dec 11 13:46:35 2020] PHP 8.0.0 Development Server (http://localhost:56522) started
```

<Info>
    Rastgele port seçme işlemi işletim sistemine göre değişiklik gösterebilir.
</Info>

<Collapsable title="Commit" titleSize="1rem">
    <a
        href="https://github.com/php/php-src/commit/a61a9fe9a0d63734136f995451a1fd35b0176292"
        target="_blank"
        rel="nofollow noopener noreferrer"
    >
